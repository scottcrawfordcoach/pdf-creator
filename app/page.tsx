...
import {
  ...
  Loader2, CheckCircle2, Image as ImageIcon, Wand2,
} from 'lucide-react'
...
const STATUS_STEPS: { key: GenStatus; label: string }[] = [
  { key: 'uploading',  label: 'Encoding brand materials' }, // Changed label
...
export default function Home() {
  ...
  const handleGenerate = useCallback(async () => {
    try {
      setStatus('uploading') // Display "Encoding..." label
      setErrorMessage(null)
      setPdfBlob(null)

      // 1. Encode brand files to Base64
      ...
      // 2. Show "analysing" state while the AI works
      setStatus('analyzing')

      // 3. Call the Python API (which runs AI + PDF generation together)
      ...
      const res = await fetch('/api/generate', {
        ...
        body: JSON.stringify({
          ...
          file_data:      fileData, // Changed from file_urls to file_data
          use_ai:         true,
        }),
      })
